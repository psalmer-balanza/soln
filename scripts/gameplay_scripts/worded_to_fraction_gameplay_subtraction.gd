...
var question_texts = [
	"Subtract these fractions: 3/4 - 1/4.",
	"Find the difference of these fractions: 25/30 - 15/25.",
	"Subtract these fractions: 1/5 - 2/5."
...
func initiate_questions():
	if DialogueState.current_quest == "raket_stealing":
		...
		question_texts = [
			"What is 4/6 - 1/6?",
			"Subtract these fractions: 4/9 - 2/6.",
			"Subtract these: 3/5 - 2/10."
		]
...
func fraction_addition_checker(first_numerator: int, first_denominator: int, second_numerator: int, second_denominator: int, answer_numerator: int, answer_denominator: int):
	if first_denominator == second_denominator:
		...
		if subtracted_numerator == answer_numerator and first_denominator == answer_denominator:
		...
		if subtracted_adjusted_numerator == answer_numerator and lcd == answer_denominator:
			$AnimationPlayer.play("spin")
